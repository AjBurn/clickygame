...
class App extends React.Component {
  state = {
    ...
    correctGuesses: 0,
    bestScore: 0,
    clickMessage: "Try to click on each hero once! Two clicks on the same hero means game over!"
  };
  ...
  handleImageClicked = async id => {
    ...
    if(clickedMatch[0].clicked) {
      ...
      this.setState({clickMessage:  "Scored a point! Great work!" });
      this.handleShuffleCards();
    } else {
      clickedMatch[0].clicked = true;
      this.state.correctGuesses = 0;
      this.state.clickMessage = "All heroes correct?? Great job!!";
